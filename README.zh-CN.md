...
cn11 修复运行中余额刷新、仅有快照或事件的日历记录，以及最终余额读取失败时的收口。新的双账本和未归属积分归因改造暂缓，不属于此版本。生产 Compose 使用 GHCR 的 `latest` 稳定标签，版本标签（当前为 `4.3.2-cn11`）用于审计和经验证的回退；部署前阅读 [修复与升级说明](docs/config-task-fix.md) 并完成一致性备份。发布镜像不会自动更新运行机，更新不需要修改 Compose 中的版本号。
...
6. 确认 cn11 工作流同时推送 `latest` 与 `4.3.2-cn11` 后，完成一致性备份，再拉取并部署 Compose 中的两个 `latest` 镜像，无须自行构建。Portainer 中先设置 `REWARDS_API_TOKEN` 环境变量，并保留原有加密密钥和数据挂载；不要让旧版镜像直接写入升级后的数据库。
