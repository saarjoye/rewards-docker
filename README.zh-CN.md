...
cn5 包含配置、调度与任务证据修复。完整 Compose 使用 `4.3.2-cn5` 镜像标签；部署前阅读 [修复与升级说明](docs/config-task-fix.md)。发布镜像不会自动更新运行机。
...
6. 确认 cn5 镜像构建成功后，拉取并部署 Compose 中的两个 `4.3.2-cn5` 镜像，无须自行构建。Portainer 中先设置 `REWARDS_API_TOKEN` 环境变量，并保留原有加密密钥和数据挂载；不能通过拉取旧 cn4 镜像获得此次修复。
