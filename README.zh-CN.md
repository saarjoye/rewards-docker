...
cn12 增加日历逐账号余额展示，以及账号开始时间、结束时间和执行时长；修复未知积分时账号结束日志无法解析的问题，并保留 cn11 实时余额与日历收口修复。账号级即时通知尚未实现，双账本和未归属积分归因改造暂缓，不属于此版本。生产 Compose 使用 GHCR 的 `latest` 稳定标签，版本标签（当前为 `4.3.2-cn12`）用于审计和经验证的回退；部署前阅读 [修复与升级说明](docs/config-task-fix.md) 并完成一致性备份。发布镜像不会自动更新运行机，更新不需要修改 Compose 中的版本号。
...
6. 确认 cn12 工作流同时推送 `latest` 与 `4.3.2-cn12` 后，完成一致性备份，再拉取并部署 Compose 中的两个 `latest` 镜像，无须自行构建。Portainer 中先设置 `REWARDS_API_TOKEN` 环境变量，并保留原有加密密钥和数据挂载；不要让旧版镜像直接写入升级后的数据库。
