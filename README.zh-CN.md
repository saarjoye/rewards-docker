...
cn8 在 cn7 基础上修复桌面端失败导致后续任务中断的问题。生产 Compose 使用 GHCR 的 `latest` 稳定标签，版本标签（当前为 `4.3.2-cn8`）仅用于回滚和审计；部署前阅读 [修复与升级说明](docs/config-task-fix.md)。发布镜像不会自动更新运行机，更新时只需执行 `docker compose pull && docker compose up -d`，不再修改 Compose 中的版本号。
...
6. 确认 cn8 工作流同时推送 `latest` 与 `4.3.2-cn8` 后，直接拉取并部署 Compose 中的两个 `latest` 镜像，无须自行构建。Portainer 中先设置 `REWARDS_API_TOKEN` 环境变量，并保留原有加密密钥和数据挂载；不能通过拉取旧 cn7 镜像获得此次修复。
