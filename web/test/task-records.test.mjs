...
test('active points and completed import deduplicate by run/account/event across midnight', () =>
    withHistory(store => {
        ...
        assert.equal(saved.collected, null)
        assert.equal(saved.accounts[0].reportedTaskPoints, 3)
...
test('different runs for the same account never overwrite historical task evidence', () =>
    withHistory(store => {
        ...
        assert.equal(store.getRun(old.id).accounts[0].reportedTaskPoints, 3)
        assert.equal(store.getRun(old.id).accounts[0].tasks[0].earnedPoints, null)
        assert.equal(store.getRun(old.id).accounts[0].initialPoints, 100)
    }))
test('account balance gain remains visible while task attribution is pending', () =>
    withHistory(store => {
        ...
        assert.equal(store.getRun('pending-run').collected, 0)
        assert.equal(store.getRun('pending-run').accounts[0].collected, 0)
        assert.equal(store.getRun('pending-run').accounts[0].runGained, null)
        assert.equal(store.getRun('legacy-run').collected, null)
        assert.equal(store.getRun('legacy-run').legacyCollected, 40)
...
test('verification migration is idempotent and preserves legacy amounts', () =>
    withHistory(store => {
        ...
        store.migrateVerification()
        assert.equal(store.getRun('old').collected, null)
        assert.equal(store.db.prepare("SELECT collected FROM runs WHERE run_key = 'old'").get().collected, 42)
