...
function renderTasks() {
    const body = (state?.accounts || [])
        .map(
            account =>
                `<article class="task-account"><h3>${esc(account.label)}</h3>${pointSummaryMarkup(account.points)}${taskTableMarkup(account.tasks, account.taskDataStatus)}</article>`
        )
        .join('')
...
accountRows = function reconciledAccountRows(accounts = []) {
    ...
    return accounts
        .map(
            account => `<tr><td>${esc(account.index)}</td><td>${esc(account.label)}</td><td>${statusPill(account.status)}<div class="subtle">${esc(account.status.message)}</div></td><td>${valueOrUnknown(account.points.balance, ' 分', '余额待确认')}</td><td>${valueOrUnknown(account.points.runGained, ' 分', '本次运行增加待确认')}</td><td>${esc(localeLabel(account.geoLocale, account.langCode))}</td></tr>`
        )
        .join('')
}
...
renderRunDetail = async function reconciledRunDetail(id) {
    ...
    try {
        ...
        displayedLogs = data.logs
        const accounts = data.run.accounts
            .map(account => `<article class="task-account"><h3>${esc(account.label)}</h3>${pointSummaryMarkup(account)}${account.pendingTaskCount ? `<p>待确认任务：${account.pendingTaskCount}</p>` : ''}${taskTableMarkup(account.tasks)}</article>`)
            .join('')
        content.innerHTML = `<section class="section"><div class="section-head"><h2>运行详情</h2><button class="ghost" data-action="history-back">返回</button></div><p>${formatTime(data.run.startedAt)} 至 ${formatTime(data.run.endedAt)} · 本次运行增加 ${valueOrUnknown(data.run.runBalanceDelta, ' 分', '待确认')} · 确认积分 ${valueOrUnknown(data.run.confirmedPoints, ' 分', '待确认')}</p><div class="task-grid">${accounts}</div></section>${logSection()}`
    } catch (error) {
        content.innerHTML = `<div class="panel empty">${esc(error.message)}</div>`
    }
}
...
renderRunDetail = async function reconciledRunDetailSummary(id) {
    ...
    try {
        const data = await api(`/api/history/${encodeURIComponent(id)}`)
        summary.textContent = `${formatTime(data.run.startedAt)} 至 ${formatTime(data.run.endedAt)} · 本次运行增加 ${valueOrUnknown(data.run.runGained, ' 分', '待确认')} · 确认积分 ${valueOrUnknown(data.run.confirmedPoints, ' 分', '待确认')}`
    } catch {}
}
