...
本次为 cn4 基线上的 cn5 源码修复。完整更新配置见 [compose.yaml](../compose.yaml)，使用 `ghcr.io/saarjoye/mrs-core:4.3.2-cn5` 和 `ghcr.io/saarjoye/mrs-web:4.3.2-cn5`。确认发布工作流成功后即可拉取，无须自行构建；发布不会自动部署运行机。
...
升级使用明确的 cn5 标签，不要把旧 cn4 标签当作本次修复。回退时恢复旧镜像及其 Compose；不要恢复或更换与现有加密数据不匹配的密钥。
