...
本次 cn11 修复运行中余额刷新、日历记录并集及最终余额收口。完整配置见 [compose.yaml](../compose.yaml)，生产使用 `ghcr.io/saarjoye/mrs-core:latest` 和 `ghcr.io/saarjoye/mrs-web:latest`；工作流同时保留 `4.3.2-cn11` 固定标签。确认发布工作流成功后即可拉取，无须修改 Compose 标签或自行构建；发布不会自动部署运行机。
