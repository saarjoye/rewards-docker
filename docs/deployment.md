...
现有仓库的 `main` 分支通过 `.github/workflows/docker-image.yml` 构建。先执行离线测试、类型检查、ESLint、构建和 Compose 校验，再构建 `linux/amd64` 与 `linux/arm64` 候选镜像。候选镜像使用空数据、禁用外部网络做启动健康检查；成功后才更新 `5.0.0-next.11` 与 `latest`。完整提交固定标签为 `sha-<完整提交号>`。

next.11 修复搜索 dashboard 进度确认：严格校验额度字段，保留最后有效进度；提交后不增长时按5/10/20秒进行有界只读复核，不重复搜索，不按提交次数推算积分。复核未确认与真实请求失败分别收口，详情显示脱敏观察时间线。搜索提交和观察摘要使用既有JSON持久化，无新增迁移，不修改历史积分。保留今日得分、账号总分、定时和通知反代；可保留数据卷回退next.10。升级后检查观察进度及部分完成状态；单次成功不保证后续搜索持续计分。
